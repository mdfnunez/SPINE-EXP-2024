...
public:
    ...
    bool setROIRectParams(int x, int y, int w, int h, bool bCheckMinMax = false);   //!< Sends new ROI to camera, while checking min/max/inc optionally
    bool setROIRectParams(const QRect &rc, bool bCheckMinMax = false);              //!< Sends new ROI to camera, while checking min/max/inc optionally
